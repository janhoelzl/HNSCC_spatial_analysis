...
def Correct_im(imDict, alphas):
    ...
    """
    Applies corrections to images based on predetermined alpha values calculated with Minimize_MI to minimize Mutual Information (MI) with previous cycles.
...
def median_alpha(inDict): 
    ...
    """
    Computes the median alpha values across all FOVs for all cycles and channels to be used for final corrections.
    ...
    -------
    outDict : dict
        A dictionary containing median MI minimizing alpha values (median across all FOVs) for every pair of cycle and previous cycle.
    """
